...
| Incomplete flows | The Dashboard has an accepted Vercel-runtime desktop partial-state capture and Enter refresh; ISS/Crew has an accepted desktop loaded-state capture and 390px keyboard-refresh check; Launches, Weather, Gallery, and Asteroids have accepted Vercel-runtime desktop loaded-state captures, with Weather reached through the 390px Watch menu; Gallery and Asteroids also completed pointer and Enter refresh while loaded. Sky Anomalies has accepted desktop start and submitted-result captures, plus a native-form semantics regression check. All flows still need complete keyboard, zoom/reflow, screen-reader, and deployed-production QA. |
