...
Apollo's dashboard is the clearest expression of its purpose—plain-English understanding of space activity now—and its partial state plus the linked Asteroids recovery state now have accepted controlled 390px evidence rather than only desktop proof.
...
| Incomplete flows | The Dashboard has accepted Vercel-runtime desktop and controlled 390px partial-state captures, Enter refresh, and a pointer route from unavailable APOD/NeoWs source cards into Apollo recovery; Asteroids’ 390px unavailable-recovery actions clear the fixed dock at the true scroll limit. ISS/Crew has an accepted desktop loaded-state capture and 390px keyboard-refresh check; Launches, Weather, Gallery, and Asteroids have accepted Vercel-runtime desktop loaded-state captures, with Weather reached through the 390px Watch menu; Gallery and Asteroids also completed pointer and Enter refresh while loaded. Asteroids’ unavailable retry also has accepted visible focus evidence. Sky Anomalies has accepted desktop and controlled 390px start/submitted-result captures, including its three loaded and one unavailable source state, plus a native-form semantics regression check. All flows still need complete keyboard, broader mobile/reflow, screen-reader, and deployed-production QA. |
...
| Evidence limit | `npm run dev` now creates the temporary copied checkout needed because Vercel's worker cannot interpret the CloudDocs path with spaces. That runtime confirmed the Dashboard partial state plus Launches, Weather, Gallery, Asteroids, and Sky Anomalies, but is not deployed-production proof. NASA's public, rate-limited `DEMO_KEY` was supplied only to the local process for the latter two flows and was not saved; the same key later returned NASA's explicit `OVER_RATE_LIMIT` response, so a dedicated non-production key is required for repeatable QA. The Dashboard partial-state capture intentionally had no NASA key, so it does not substitute for full-live Dashboard evidence. A current controlled 390 × 844 CSS viewport reported `innerWidth`, `clientWidth`, and `scrollWidth` of 390; its Dashboard partial, Asteroids unavailable-recovery, and Sky Anomalies start/submitted-result states are accepted, while full mobile coverage and hardware checks remain open. Native mobile Watch keyboard activation remains unproven because the current controller did not open it with Enter or Space. The same controller focused Sky Anomalies radios and submit without activation, and focused Asteroids' retry without dispatching Enter or Space through either available key channel. Focus-ring evidence is accepted; no key-activation claim is made. Unavailable-state screenshot output that did not match the visible DOM remains rejected as visual evidence. |
