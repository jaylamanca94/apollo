...
Use compact primary header links once Apollo has multiple meaningful pages. Keep page links next to the brand, use Acadia's no-capsule header nav treatment with 8px active/hover targets and `aria-current="page"`, use a simple live-data chip for the public data state, and preserve the theme toggle as a utility control instead of mixing it into page navigation. On phone widths, collapse those same destinations into Acadia's floating glass dock with icon-first tabs. Keep the header to five primary items maximum; use `More` for overflow. Weather should remain a first-class header destination because space weather is one of Apollo's clearest differentiators.
When an overflow destination is the current page, the overflow toggle should identify that destination with its own short label and icon instead of showing a generic `More`/ellipsis state. This keeps the mobile dock's active item recognizable while preserving the five-item navigation limit.
