...
- Space Brief: place it immediately after the page header, before quick stats. Lead with one useful topic, its explicit observation/publication/target time and source, a short interpretation or limitation, and one Acadia secondary-button link into detail. Close with loaded/checking/limited/unavailable coverage across all six sources. Do not infer a global Calm/Active or normal-orbit verdict from this limited collection. Keep recommendation ordering and scientific interpretation local to Apollo. Preserve action focus through incremental source resolution; if the destination changes, focus the new headline and leave its action next in tab order.
