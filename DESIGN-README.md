...
- Sky Anomalies detail page: keep the sighting context form compact and the result language sourced, neutral, and careful. Use check/describe language until Apollo actually submits reports, and use fit/evidence language instead of probability-style confidence claims. `strong`, `possible`, `context`, and `unknown` are acceptable result states, while speculative origin language is not. UAP and fireball report feeds should appear as planned source gaps until real imports exist.
...
Use one global refresh timestamp near the page title. Avoid repeated per-card timestamps and section jump navigation on single-page dashboard views. Reserve the largest hero treatment for the Dashboard; object pages should use compact section headers so the page-specific tool or content appears sooner.
Use compact primary header links once Apollo has multiple meaningful pages. Keep page links next to the brand, use Acadia's no-capsule header nav treatment with 8px active/hover targets and `aria-current="page"`, use a simple live-data chip for the public data state, and preserve the theme toggle as a utility control instead of mixing it into page navigation. On phone widths, collapse those same destinations into Acadia's floating glass dock with icon-first tabs. Keep the header to exactly five primary items: Dashboard, ISS, Launches, Watch, and Gallery.
Use Watch as the labeled group for monitoring destinations: Weather, Asteroids, and Anomalies. Do not use a generic `More`/ellipsis nav item; grouped navigation should describe the information inside it so new users can infer what it contains before opening it. When a Watch destination is current, keep the Watch trigger active and the matching menu item marked with `aria-current="page"`.
