...
- Sky Anomalies detail page: keep the sighting context form compact and the result language sourced, neutral, and careful. Use check/describe language until Apollo actually submits reports, and use qualitative evidence labels instead of numeric fit, probability, confidence, identity, or verification claims. State browser-local time handling and clarify that typed locations are descriptive until location-aware matching exists. `strong`, `possible`, `context`, and `unknown` are acceptable result states, while speculative origin language is not. UAP, fireball, flight-tracking, planet-position, and satellite-visibility feeds should appear as planned source gaps until real imports exist.
...
Use one global freshness label near the page title. Use "Last updated" only after source data loads successfully; use "Last checked" when a page is partial, degraded, or unavailable. Avoid repeated per-card timestamps and section jump navigation on single-page dashboard views. Reserve the largest hero treatment for the Dashboard; object pages should use compact section headers so the page-specific tool or content appears sooner.
Use compact primary header links once Apollo has multiple meaningful pages. Keep page links next to the brand, use Acadia's no-capsule header nav treatment with 8px active/hover targets and `aria-current="page"`, use a public-data chip that can say live, partial, or unavailable, and preserve the theme toggle as a utility control instead of mixing it into page navigation. On phone widths, collapse those same destinations into Acadia's floating glass dock with icon-first tabs and keep enough bottom content clearance for the dock, its gap, and safe-area inset. Keep the header to exactly five primary items: Dashboard, ISS, Launches, Watch, and Gallery.
