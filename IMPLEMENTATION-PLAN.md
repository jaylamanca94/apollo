...
Reliability Update:
APOD and Near-Earth Object API responses are now normalized server-side before reaching the browser, matching the Launch Library proxy pattern. Frontend normalizers still accept older raw NASA shapes as a compatibility fallback. Fixture tests now cover the normalized APOD, launch, and Near-Earth Object contracts.
