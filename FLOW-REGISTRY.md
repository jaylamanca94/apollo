...
**Registry status:** Seven canonical user goals verified in code. Every affected detail flow now keeps source-specific recovery with the unavailable data; fresh loaded-state, visual, keyboard, zoom, and assistive-technology QA remain incomplete.
...
- ISS/Crew, Launches, Weather/Asteroids, and Gallery still lack current loaded-state evidence from their live upstream sources; the retry reduces a dead end but does not create a resilience policy.
